...
const copyPortal = async () => {
    await navigator.clipboard.writeText(props.consignor.portal_url);
    copied.value = true;
    setTimeout(() => (copied.value = false), 2000);
};
const resetPortal = () => confirm('Cabut semua link portal lama untuk penitip ini?') && router.post(route('admin.consignors.reset-portal', props.consignor.id), {}, { preserveScroll: true });
const waLink = computed(() => `https://wa.me/${(props.consignor.phone || '').replace(/^0/, '62').replace(/\D/g, '')}?text=${encodeURIComponent(`Halo ${props.consignor.name}, pantau status barang titipan Anda di: ${props.consignor.portal_url}`)}`);
</script>
    ...
    <AdminLayout :title="`${consignor.code} — ${consignor.name}`">
        <div class="grid gap-6 lg:grid-cols-[340px_1fr]">
            <section class="card h-fit p-6 text-sm">
                ...
                <div class="mt-6 rounded-xl bg-stone-50 p-3">
                    <p class="font-medium text-ink">🔗 Portal penitip</p>
                    <p class="mt-1 text-xs text-stone-500">Link read-only untuk penitip memantau barang & settlement. Berlaku {{ consignor.portal_days }} hari.</p>
                    <div class="mt-2 flex flex-wrap gap-2">
                        <button class="btn-dark btn-sm" @click="copyPortal">{{ copied ? '✓ Tersalin' : 'Salin link' }}</button>
                        <a v-if="consignor.phone" :href="waLink" target="_blank" rel="noopener" class="btn-outline btn-sm">Kirim via WA</a>
                        <button class="btn-outline btn-sm text-red-600" @click="resetPortal">Cabut link lama</button>
                    </div>
                </div>
