    ...
    <Link :href="route('lots.show', lot.id)" class="group card flex flex-col overflow-hidden transition hover:-translate-y-0.5 hover:shadow-lg">
        <div class="relative aspect-[4/3] overflow-hidden bg-stone-100">
            ...
            <div class="absolute top-3 left-3 flex flex-wrap gap-2">
                ...
                <StatusBadge v-if="lot.method && lot.method.value !== 'open'" :status="lot.method" />
            </div>
            ...
            <span v-if="lot.is_leader === null" class="absolute bottom-3 left-3 rounded-full bg-purple-700 px-2.5 py-0.5 text-xs font-bold text-white">
                ✉️ Penawaran tertutup terkirim
            </span>
            <span v-else-if="lot.is_leader !== undefined" class="absolute bottom-3 left-3 rounded-full px-2.5 py-0.5 text-xs font-bold"
                :class="lot.is_leader ? 'bg-green-600 text-white' : 'bg-red-600 text-white'">
                {{ lot.is_leader ? 'Anda memimpin' : 'Anda terlampaui' }}
            </span>
            <span v-if="lot.buy_now_price" class="absolute right-3 bottom-3 rounded-full bg-emerald-600 px-2.5 py-0.5 text-xs font-bold text-white">
                ⚡ Beli langsung
            </span>
        </div>
        <div class="flex flex-1 flex-col gap-3 p-4">
            ...
            <div class="mt-auto flex items-end justify-between gap-2">
                <div>
                    <p class="text-xs text-stone-500">{{ lot.bids_count === null ? 'Harga awal · tertutup' : lot.bids_count ? `${lot.bids_count} penawaran` : 'Harga awal' }}</p>
                    <p class="text-lg font-bold text-ink">{{ money(lot.current_price) }}</p>
                </div>
                <div class="text-right text-xs text-stone-500">
                    <template v-if="lot.status.value === 'live' && lot.method?.value === 'live'">
                        <p class="font-semibold text-red-600">🎙️ Live</p>
                    </template>
                    <template v-else-if="lot.status.value === 'live'">
                        <p>Berakhir</p>
